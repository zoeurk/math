...
options[] = {	{"double",'d', NULL,0,"uliser une valeur double", 0},
		...
		{"floor",'o',"X",0,"arrondi un nombre X a ma plus grande valeur intégrale non supérieure à l'argument", 1},
		...
		{"idegres",'i',NULL,0,"les entrées sont converti en radian",3},
 		{"odegres",'x',NULL,0,"calcule l'angle en degrès si l'entree est converti en radian",3},
  ...
  case POW: case FMOD:
   switch(set)
   {
    case DOUBLE:  calcule.result.dresult = calcule.d.dfn2(calcule.value.dnumber[0],calcule.value.dnumber[1]);
    		  if((args.type&IDEGRES) == IDEGRES && (args.type&ODEGRES) == ODEGRES)
